...
public class EnableNotificationCallbackHandler : ICallbackHandler
{
    ...
    public async Task<(Message Message, Conversation? Conversation)> HandleCallbackAsync(CallbackQuery callback, CancellationToken cancellationToken = default)
    {
        ...
        if (all)
        {
            _logger.LogDebug("Enabling all notifications for chat {ChatId}", chatId);
            await _notificationScheduleService.EnableAllByUserIdAsync(user.Id, cancellationToken);
            _logger.LogDebug("All notification of chat {ChatId} were enabled", chatId);
            return (await _bot.EditMessageText(chatId: chatId, messageId: messageId, text: "Enabled all notifications.", cancellationToken: cancellationToken), null);
        }
